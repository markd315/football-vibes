...
function renderAssignments() {
    ...
    selectedPlayers.forEach((playerId) => {
        ...
        // Group by position: QB + eligibles (QB, RB, FB, WR, TE) vs linemen (OT, OG, C)
        if (['QB', 'RB', 'FB', 'WR', 'TE'].includes(player.position)) {
...
function applyDefensivePlaycall(playcallName) {
    ...
    // Clear ALL existing assignments for selected defenders to prevent stale "Man Coverage"
    selectedDefense.forEach(id => {
        const p = getPlayerById(id);
        if (p) {
            assignments.defense[p.name] = { category: '', action: '', manCoverageTarget: null };
        }
    });
    ...
    if (useMan) {
        ...
        if (coverNumber === 2) {
            ...
            // Outside CBs (widest) get Flat
            const leftCB = cbsWithPositions[0];
            const rightCB = cbsWithPositions[cbsWithPositions.length - 1];
            if (leftCB) updateAssignment(leftCB.cb, 'defense', 'Zone Short', 'Flat L');
            if (rightCB && rightCB !== leftCB) updateAssignment(rightCB.cb, 'defense', 'Zone Short', 'Flat R');

            // Remaining CBs (Nickel) and other DBs get Curl/Hook
            const curlHookDBs = [
                ...cbsWithPositions.filter(item => item !== leftCB && item !== rightCB).map(i => i.cb),
                ...otherDBs
            ];
            curlHookDBs.forEach((db, idx) => {
                updateAssignment(db, 'defense', 'Zone Short', idx % 2 === 0 ? 'Curl/Hook L' : 'Curl/Hook R');
            });
        } else {
            remainingDBs.forEach((db) => {
...
async function executePlay() {
    ...
    // Ensure baseline rates are loaded (fix for persistence issue)
    if (!baselineRates) {
        console.warn('Baseline rates missing, reloading...');
        await loadBaselineRates();
    }
